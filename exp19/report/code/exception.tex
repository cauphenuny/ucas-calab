...
\begin{enumerate}
    ...
    \item \textbf{异常优先级}：多个异常可能同时触发时，按照优先级择一处理。
\end{enumerate}
...
\begin{enumerate}
    \item nextPC经过三级地址翻译得到物理PC
    \item 同时进行异常检测，生成取指异常标志
    \item 将异常信息和物理地址送往IF级
\end{enumerate}

\begin{lstlisting}[language=Verilog, caption=取指地址转换集成]
// Pre-IF stage: address translation for next PC
wire [31:0] nextpc_pa;
wire if_tlb_excep;
...
mmu_inst_translate(
    ...
    .dmw0(csr_dmw0),
    .dmw1(csr_dmw1),
    .tlb_result(tlb_s0_result),  // TLB search port 0
...
// Use translated address for instruction fetch request
assign inst_sram_addr = nextpc_pa;
\end{lstlisting}

\textbf{访存阶段的地址转换}

在EX级进行load/store虚实地址转换：

\begin{enumerate}
    \item 计算得到的虚地址经过三级翻译得到物理地址
    \item 同时进行异常检测
    \item 将物理地址送往MEM级用于访存请求
\end{enumerate}

\begin{lstlisting}[language=Verilog, caption=访存地址转换集成]
// EX stage: address translation for load/store
wire [31:0] data_vaddr;  // Virtual address calculated in EX
...
mmu_mem_translate(
    ...
    .dmw0(csr_dmw0),
    .dmw1(csr_dmw1),
    .tlb_result(tlb_s1_result),  // TLB search port 1
    
    .paddr(data_paddr),
    .excep(mem_tlb_excep),
    .excep_code(mem_ecode)
);

// Only issue memory request if no exception
assign mem_req_valid = valid && !mem_tlb_excep;
...
\begin{enumerate}
    \item \textbf{禁止发送非法地址}：若地址转换产生异常，不能将无意义的物理地址发往总线，防止系统行为不可控。
    
    \item \textbf{异常优先处理}：异常生成的指令不能产生任何执行效果，必须在WB级清空流水线并跳转到异常处理程序。
    
    \item \textbf{时序考虑}：地址转换逻辑会增加Pre-IF和EX级的组合逻辑延迟。可在后续优化中采用流水线化等手段改善。
\end{enumerate}
